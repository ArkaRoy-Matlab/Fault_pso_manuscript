% ***************************************************************
% *** Matlab code for Parameter tuning of number of population for PSO   
...
%% Code for Parameter tuning of number of population for PSO  
clear all
close all
...
%Run Model for 10 times and taking best model out of this 10 independent runs
    for ii=1:10
        ...
        %Social and cognitive component variation
        c1=1; c2=1.2;
        nVar=4;                %Number of Unknown Variable including density
        MaxIt = 300;           %Maximum number of iterations
        nPoP =  nn_pop(N_pp);  %Population size or swarm size
...
[val,id]=sort(all_best(:,3));
mean_beast=mean(all_best(id(1:5),:));
costt(N_pp)=mean_beast(1);
timee(N_pp)=mean_beast(2);
iterr(N_pp)=floor(mean_beast(3));
%printing results for all npop
fprintf('For nPoP=%2.2d, best cost =%2.2e, best time==%2.4f and number of iteration=%d\n',nPoP,costt(N_pp),timee(N_pp),iterr(N_pp))
